...
1. Scan recent logs for entries worth promoting to long-term memory:
   - New lessons learned
   - Topics covered (articles, digests)
   - Important findings from monitors
   - Ideas captured that are still relevant
   - Goals completed or progress milestones
2. Check each candidate against existing MEMORY.md — skip if already recorded.
3. Update memory:
   - Add brief entries to MEMORY.md (keep it under ~50 lines)
   - If a topic needs more detail, write to memory/topics/<topic>.md instead
   - Update tables with new rows
4. Do NOT rewrite the whole file — make targeted additions only.
